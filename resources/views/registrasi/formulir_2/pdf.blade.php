...
<html lang="en" dir="ltr">
  ...
  <div class="clear"></div>
    ...
    <h3>FORMULIR PEDAFTARAN {{strtoupper($row->get_kategori_perjalanan->deskripsi)}} TAHUN {{date('Y',strtotime($row->tgl_registrasi))}}</h3>
  </div>
      <div id="head_1">
        <table>
          <tr>
...
<div id="left">
  ...
  <thead>
    <tr>
      <thead>
        <th colspan="2">Paket Program</th>
      </thead>
    </tr>
    <tbody>
      <tr>
        <td>Paket Yang Dipilih</td>
        <td>{{$row->get_paket_perjalanan->nama_paket}}</td>
      </tr>
      <tr>
        <td>Sub Paket</td>
        <td>{{$sub_paket->kapasitas->deskripsi}}</td>
      </tr>
      <tr>
        <td>Program Yang Dipilih</td>
        <td>{{date('d-m-Y',strtotime($row->program))}}</td>
      </tr>
      <tr>
...
<p style="margin-top:-5px"></p>
<div id="left_full">
  <font style="font-size:12px;font-weight:bold">Keluarga Yg Ikut Bersama </font>
  <p style="margin-top:-5px"></p>
  ...
  <thead>
    ...
    <tbody>
      @foreach($keluarga_ikut as $row_item)
      <tr>
        <td>{{$row_item->nama}}</td>
        <td>{{$row_item->get_hubungan->deskripsi}}</td>
        <td>{{$row_item->tlp}}</td>
      </tr>
      @endforeach
    </tbody>
