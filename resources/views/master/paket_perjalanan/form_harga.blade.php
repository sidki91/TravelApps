...
function save_item(key)
{
    $.ajax({
        type: 'POST',
        url: 'paket_perjalanan/save_harga_paket',
        data: {
...
<table class="table table-striped">
  ...
  <tbody>
    ...
    <?php
    $harga = \App\Models\HargaPaketTemp::where('kode_kapasitas',$row->kode_kapasitas)
    ...
    <tr>
      ...
      <td>
        <input type="text" class="form-control" id="harga_{{$urutan}}" value="{{$harga}}" onkeyup="money_format('{{$no}}')"/>
        <input type="hidden" id="harga_val_{{$urutan}}"/>
      </td>
